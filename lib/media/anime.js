...
class Nimegami {
  ...
  async _fetch(url) {
    const res = await axios.get(url, { headers: this.defaultHeaders, timeout: 30000 });
    return res.data;
  }
  ...
  async detail(url) {
    ...
    return { title, synopsis, video, downloads: downloads.slice(0, 8) };
  }
}

module.exports = Nimegami;