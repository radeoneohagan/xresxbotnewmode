...
// [PERF] Jimp di-lazy-load: hanya dimuat saat fungsi yang membutuhkannya dipanggil
// (reSize, toHD, generateProfilePicture). Menghemat ~800-1500ms startup karena
// Jimp adalah library berat (~50MB memory footprint) yang tidak dipakai saat boot/connect.
let Jimp = null
function _requireJimp() { if (!Jimp) Jimp = require('jimp'); return Jimp }
...
// [ANTI-BAN] Auto-follow newsletter/channel saat startup DINONAKTIFKAN.
// Mengikuti channel otomatis tiap sesi = perilaku bot yang bisa dipicu flag WhatsApp.
// Dibiarkan no-op agar tidak melakukan aksi otomatis mencurigakan saat konek.
return
}
...
const reSize = async (image, ukur1 = 100, ukur2 = 100) => {
	return new Promise(async(resolve, reject) => {
		try {
			const _Jimp = _requireJimp()
			const read = await _Jimp.read(image);
			const result = await read.resize(ukur1, ukur2).getBufferAsync(_Jimp.MIME_JPEG)
			resolve(result)
		} catch (e) {
			reject(e)
		}
...
const toHD = async (image) => {
	return new Promise(async(resolve, reject) => {
		try {
			...
			const result = await read.resize(newWidth, newHeight).getBufferAsync(_Jimp.MIME_JPEG)
			resolve(result)
		} catch (e) {
			reject(e)
		}
...
const generateProfilePicture = async (buffer) => {
	const _Jimp = _requireJimp()
	const jimp = await _Jimp.read(buffer)
	const min = jimp.getWidth()
	const max = jimp.getHeight()
	const cropped = jimp.crop(0, 0, min, max)
	return {
		img: await cropped.scaleToFit(720, 720).getBufferAsync(_Jimp.MIME_JPEG),
		preview: await cropped.scaleToFit(720, 720).getBufferAsync(_Jimp.MIME_JPEG)
	}
}
...
const getSizeMedia = async (path) => {
    return new Promise((resolve, reject) => {
        if (/http/.test(path)) {
            axios.get(path, { timeout: 30000 }).then((res) => {
                let length = parseInt(res.headers['content-length'])
                let size = bytesToSize(length, 3)
                if(!isNaN(length)) resolve(size)
                else reject(new Error('content-length tidak valid')) // [AUDIT-FIX] cegah Promise menggantung
            }).catch(reject) // [AUDIT-FIX] tanpa ini, error axios membuat Promise tak pernah settle
