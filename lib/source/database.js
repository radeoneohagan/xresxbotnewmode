...
if (/mongo/.test("database.json")) {
	...
	DataBase = class dataBase {
		...
		write = async (data) => {
			...
			// [PERF] Skip bila tidak ada perubahan sejak penulisan terakhir -> hindari I/O sia-sia.
			const jsonStr = JSON.stringify(this.data, null, 2)
			if (jsonStr === this._lastStr) return this.file
			// [PERF] Tulis ASYNC (fs.promises) agar TIDAK memblok event-loop (fix lag berkala
			// tiap interval simpan). Tetap atomic: tulis ke .tmp lalu rename.
			const tmpFile = this.file + '.tmp'
			try {
				await fs.promises.writeFile(tmpFile, jsonStr)
				// Backup file valid sebelumnya (async)
				if (fs.existsSync(this.file)) {
					try { await fs.promises.copyFile(this.file, this.backupFile) } catch {}
				}
				await fs.promises.rename(tmpFile, this.file)
				this._lastStr = jsonStr
			} catch (e) {
